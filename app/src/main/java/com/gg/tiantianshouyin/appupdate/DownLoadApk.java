...
public class DownLoadApk {
    ...
    public static void download(Context context, String url, String title, final String appName) {
        ...
        if (downloadId != -1L) {
            ...
            if (status == DownloadManager.STATUS_SUCCESSFUL) {
                ...
                start(context, url, title,appName);       //下载到本地
    ...
    /**
     * start里面执行FileDownloadManager的startDownload方法。
     * @param context
     * @param url
     * @param title
     * @param appName
    ...
    private static PackageInfo getApkInfo(Context context, String path) {
        ...
        PackageInfo info = pm.getPackageArchiveInfo(path, PackageManager.GET_ACTIVITIES);    // archive 档案
        if (info != null) {
            return info;
        }
        return null;
